...
class TemplateStore(object):
    ...
    def __init__(self, file_name, probe, mode='w', two_components=False):
        ...
        if self.mode == 'w':
            ...
            for channel, indices in self.probe.edges.items():
                ...
                self.h5_file.create_dataset('mapping/%d', data=indices, chunks=True)
            self.h5_file.create_dataset('indices', data=numpy.zeros(0, dtype=numpy.int32))
            self.h5_file.close()
        ...
        for key in templates_data['dat'].keys():
            for channel in templates_data['dat'][key].keys():
                ...
                for count in xrange(len(templates)):
                    gidx = self.get_next_index() 

                    self.h5_file.create_dataset('templates/1/%d' %gidx, data=templates[count], chunks=True, maxshape=(None,))
                    template_norm = np.sqrt(np.sum(templates[count]**2)/self._nb_elements)
                    self.h5_file.create_dataset('norms/1/%d' %gidx, data=template_norm, chunks=True, maxshape=(None,))

                    if self.two_components:
                        self.h5_file.create_dataset('templates/2/%d' %gidx, data=templates2[count], chunks=True, maxshape=(None,))
                        template_norm = np.sqrt(np.sum(templates2[count]**2)/self._nb_elements)
                        self.h5_file.create_dataset('norms/2/%d' %gidx, data=template_norm, chunks=True, maxshape=(None,))                    
