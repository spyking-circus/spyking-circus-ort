...
class Density_clustering(Block):
    ...
    def _process(self):
        if self.receive_pcs:
            ...
            self.data_points = {}
            for channel in xrange(self.nb_channels):
                self.data_points[channel] = numpy.zeros((self.pcs.shape[1], 0), dtype=numpy.float32)
            ...
            for channel, signed_peaks in peaks[key].items():
                for peak in signed_peaks:
                    if self._is_valid(peak):
                        ...
                        self.data_points[channel] = numpy.hstack((self.data_points[channel], projection))
