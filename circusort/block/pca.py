...
class Pca(Block):
    ...
    def _initialize(self):
        self.nb_timesteps = int(self.sampling_rate*self.N_t*1e-3)
        self.count = 0
        if numpy.mod(self.nb_timesteps, 2) == 0:
            self.nb_timesteps += 1
        self.width        = (self.nb_timesteps-1)//2
        return

    def _guess_output_endpoints(self):
        self.output.configure(dtype=self.inputs['data'].dtype, shape=(self.nb_channels, self.nb_samples))
        self.waveforms = numpy.zeros((self.nb_waveforms, self.nb_timesteps), dtype=numpy.float32)
