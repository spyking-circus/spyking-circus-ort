...
cluster_1     = manager[slaves[0]].create_block('density_clustering', probe=probe_file, nb_waveforms=100, two_components=two_components)
...
director.connect(whitening.output, [mad_estimator.input, peak_detector.get_input('data'), cluster_1.get_input('data'), pca.get_input('data'), fitter.get_input('data'), writer.input])
director.connect(mad_estimator.output, [peak_detector.get_input('mads'), cluster_1.get_input('mads'), writer_4.input])
director.connect(peak_detector.get_output('peaks'), [pca.get_input('peaks'), cluster_1.get_input('peaks'), fitter.get_input('peaks'), writer_3.input])
director.connect(pca.get_output('pcs'), [cluster_1.get_input('pcs')])
