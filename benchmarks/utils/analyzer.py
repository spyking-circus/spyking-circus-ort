...
class Analyzer(object):
    ...
    def __init__(self, spk_writer_params, probe, template_store, synthetic_file=None, filtered_data=None):
        ...
        if self.filtered_data is not None:
            self.raw_data = numpy.fromfile(filtered_data, dtype=numpy.float32)
            self.raw_data = self.raw_data.reshape(self.raw_data.size/self.nb_channels, self.nb_channels)
        else:
            self.raw_data = None
